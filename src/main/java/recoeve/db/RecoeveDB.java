...
public class RecoeveDB {
	...
	public RecoeveDB(Vertx vertx) {
		...
		try {
			...
			pstmtPutRedirect = con
					.prepareStatement("INSERT INTO `redirect` (`hashpath`, `originalURI`) VALUES (?, ?);");
			...
			pstmtCreateAuthToken = con
					.prepareStatement("INSERT INTO `AuthToken` (`t`, `ip`, `token`) VALUES (?, ?, ?);");
			...
			// pstmtDeleteUser = con.prepareStatement("DELETE FROM `Users` WHERE
			// `email`=?;");
		...
		try {
			...
			for (int i = 1; i < iMax; i++) {
				...
				if (host == null || host.equals("null")) {
					...
					while (rs.next()) {
						stats += "\n" + rs.getString("t") + "\t" + rs.getString("ip") + "\t" + rs.getString("URI")
								+ "\t" + rs.getString("referer") + "\t" + rs.getString("REACTION_GUEST");
					}
			...
			if (pstmtCreateUser.executeUpdate() > 0) {
				...
				if (user.next()) {
					...
					logsCommit(user.getLong("i"), tNow, ip, "snu", true,
							"With Google :: ID: " + id + ", E-mail: " + email); // sign-up
					done = true;
				}
			}
		} catch (SQLException e) {
					...
					try {
						...
						if (rs.next()
								&& Arrays.equals(rs.getBytes("encryptedSSN"),
										pwdEncrypt(rs.getBytes("salt"), Encrypt.encryptSSNRest(
												bytesToHexString(rs.getBytes("salt")), session, rs.getInt("iter"))))) {
				...
				if (!data.get(1, "id").isEmpty()) {
					if (createUserWithGoogle(data, inputs, ip, tNow)) {
						...
						if (user != null && user.next()) {
							...
							byte[] rmbdToken = randomBytes(32);
							setCookie.addAll(
									createUserRemember(user_me, tNow, rmbdAuth, rmbdToken, inputs, ip, userAgent));
							logsCommit(user_me, tNow, ip, "lgi", true, "Remembered with Google"); // log-in success
							done = true;
						} else {
							logsCommit(user_me, tNow, ip, "lgi", false,
									"User does not exist. Email:" + inputs.get(1, "email")); // log-in fail
							done = true;
						}
					}
				} else {
		...
		try {
			...
			if (user != null && user.next()) {
				...
				if (Arrays.equals(
					...
					System.out.println(bytesToHexString(pwdEncrypt(salt,
							Encrypt.encryptRest(bytesToHexString(salt), inputs.get(1, "userPwd"), iter))));
					System.out.println(bytesToHexString(user.getBytes("pwd")));
					logsCommit(user_me, tNow, ip, "lgi", false); // log-in fail
				}
			} else {
				logsCommit(user_me, tNow, ip, "lgi", false); // user_me=1: anonymous (no id/email) log-in try. This must
																// not happen, because of "account/pwd_iteration" check
					...
					if (rs.next()) {
						...
						if ( /*
							...
							if (user != null && user.next()) {
								...
								setCookie.add(io.vertx.core.http.Cookie.cookie("rmbdToken", bytesToHexString(newToken))
										.setSecure(true)
	...
	public List<io.vertx.core.http.Cookie> createUserSession(long user_me, Timestamp tNow, byte[] session, byte[] salt,
		...
		try {
			...
			if (user.next()) {
				...
				NeighborList neighborList = new NeighborList(
						neighborListFrom.toStringRowMap() + neighborListTo.toStringRowMap(), false, true);
									...
									if (revNeighborListFrom.mapArray.putIfAbsent(
										...
										if (revRSneighborListFrom != null) {
											revRSneighborListFrom.updateString("userCatList",
													revNeighborListFrom.toStringRowMap());
											revRSneighborListFrom.updateTimestamp("tUpdate", tNow);
											revRSneighborListFrom.updateRow();
										} else {
											putNeighborListFrom(Long.parseLong(aL.get(0), 16), aL.get(1),
						...
						for (int k = 0; k < uris.length; k++) {
							...
							if (!reco_from.next()) {
								...
								if (neighborReco.next()) {
									res += "\n" + neighborJStr + "\t" + user_id + "\t" + StrArray.enclose(uri) + "\t"
											+ StrArray.enclose(neighborReco.getString("cats")) + "\t"
											+ neighborReco.getString("val") + "\t"
											+ StrArray.enclose(neighborReco.getString("cmt")) + "\t"
											+ neighborReco.getTimestamp("tLast").toString();
								}
							}
						}
					}
	...
	// pstmtPutRecentests=con.prepareStatement("INSERT INTO `RecoStat` (`uri`,
	// `recentests`, `tFirst`, `tUpdate`, `N`) VALUES (?, ?, ?, ?, 1);");
		...
		if (rs.next()) {
			...
			try {
				if (recentests != null) {
					...
					if (rsL >= 8) {
						equalityOfRecentest = Arrays.equals(Arrays.copyOfRange(recentests, rsL - 8, rsL),
								user_me_bytes);
				...
				for (int i = recentestsArray.length - 1; i >= 0; i--) {
					...
					if (recentestsSet.add(rI)) {
						...
						if (user.next()) {
							...
							if (reco.next()) {
								recentests += "\n" + Long.toString(rI, 16) + "\t" + user.getString("id") + "\t"
										+ reco.getString("val") + "\t" + StrArray.enclose(reco.getString("cmt"));
							}
						}
					}
				}
				heads += "\trecentests\ttFirst\ttUpdate\tsumV100\tnV";
				contents += "\t" + StrArray.enclose(recentests) + "\t" + rs.getString("tFirst") + "\t"
						+ rs.getString("tUpdate") + "\t" + Long.toString(rs.getLong("sumV100"), 16) + "\t"
						+ Long.toString(rs.getLong("nV"), 16);
				for (int i = 0; i <= 100; i++) {
					heads += "\tn" + i;
	...
	// INSERT INTO `Neighbors` (`user_i`, `cat_i`, `user_from`, `cat_from`,
	// `sumSim`, `nSim`, `tUpdate`, `tScanAll`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
	public boolean putNeighbor(long user_to, String cat_to, long user_from, String cat_from, long sumSim, int nSim,
			Timestamp tNow, Timestamp tScanAll) throws SQLException {
	...
	public boolean putNeighbor(long user_to, String cat_to, long user_from, String cat_from, long sumSim, int nSim,
			Timestamp tNow) throws SQLException {
		pstmtPutNeighbor.setLong(1, user_to);
		pstmtPutNeighbor.setString(2, cat_to);
		pstmtPutNeighbor.setLong(3, user_from);
		pstmtPutNeighbor.setString(4, cat_from);
	...
	public boolean putNeighborListFrom(long user_from, String cat_from, NeighborList userCatList, Timestamp tNow)
			throws SQLException {
	...
	public boolean updateNeighborListFrom(long user_from, String cat_from, NeighborList userCatList)
			throws SQLException {
	...
	public boolean updateNeighborListFrom(long user_from, String cat_from, NeighborList userCatList, Timestamp tNow)
			throws SQLException {
	...
	public boolean putNeighborListTo(long user_to, String cat_to, NeighborList userCatList, Timestamp tNow)
			throws SQLException {
	...
	public boolean updateNeighborListTo(long user_to, String cat_to, NeighborList userCatList, Timestamp tNow)
			throws SQLException {
	...
	public void putNeighborWithScanAll(long user_to, String cat_to, long user_from, String cat_from, Timestamp tNow)
			throws SQLException {
		ResultSet neighbor = getNeighbor(user_to, cat_to, user_from, cat_from);
		if (neighbor.next()) {
	...
	public void updateNeighbors(long user_from, String uri, Categories cats, Points pts, CatList catL, Timestamp tNow,
			...
			if (increment == 1) {
				...
				for (String cat_from : cats.setOfCats) {
					...
					for (int i = 0; i < n; i++) { // 이미 계산된 neighborList 에서 1개 new URI update 하기.
						if (neighborList.mapArray
								.remove(neighborList.get(i, 0) + "\t" + neighborList.get(i, 1)) != null) {
									...
									if (cats_to.contains(cat_to)) {
										if (nbExists && nbRevExists) {
											...
											if (tNow.getTime() - nbTScanAll.getTime() > HALF_YEAR_IN_MS) {
												...
												Timestamp tScanAll = neighbor.getTimestamp("tScanAll");
												neighbor.updateRow();
												putNeighbor(user_from, cat_from, user_to, cat_to, sim.sumSim, sim.nSim,
														tNow, tScanAll);
											}
										} else if (nbRevExists) {
											if (tNow.getTime() - nbRevTScanAll.getTime() > HALF_YEAR_IN_MS) {
												...
												Timestamp tScanAll = neighborRev.getTimestamp("tScanAll");
												neighborRev.updateRow();
												putNeighbor(user_to, cat_to, user_from, cat_from, sim.sumSim, sim.nSim,
														tNow, tScanAll);
											}
										} else { // ScanAll
											putNeighborWithScanAll(user_to, cat_to, user_from, cat_from, tNow);
	...
	public void deleteCatsUriFromList(long user_me, Categories cats, String uri, CatList catL) throws SQLException {
		// Reco 를 지우거나, Reco cats 를 바꿀때 빈 cat 들은 자동으로 지우도록. 지워졌을 경우 superCat 들도 비었는지
		// check 하면서 지워나가야 함.
		for (String cat : cats.setOfCats) {
			pstmtGetUriList.setLong(1, user_me);
			pstmtGetUriList.setString(2, cat);
	...
	public void catsChangedOnUri(long user_me, Categories oldCats, Categories newCats, String uri, CatList catL,
			boolean equalityOfValuesOfPts, Points oldPts, Points newPts, Timestamp tNow) throws SQLException {
						...
						case "change":
							...
							if (equalityOfStringOfCats && equalityOfTitle && equalityOfDesc && equalityOfCmt
								...
								if (!equalityOfStringOfCats) {
									...
									if (!equalityOfCats) {
										catsChangedOnUri(user_me, oldCats, cats, uri, catL, equalityOfValuesOfPts,
												oldPts, pts, tNow);
									}
								}
								if (!equalityOfTitle) {
	...
	public void updateDefsAll(Timestamp tNow) {
		try {
			PreparedStatement pstmtGetAllRecos = con.prepareStatement("SELECT * FROM `Recos1`;",
					ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
	...
	public void moveRecosToRecos1() {
		try {
			PreparedStatement pstmtGetAllRecos = con.prepareStatement("SELECT * FROM `Recos`;",
					ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			ResultSet rs = pstmtGetAllRecos.executeQuery();
			// pstmtPutReco=con.prepareStatement("INSERT INTO `Recos1` (`user_i`, `uri`,
			// `tFirst`, `tLast`, `cats`, `title`, `desc`, `cmt`, `val`) VALUES (?, ?, ?, ?,
			// ?, ?, ?, ?, ?);");
			while (rs.next()) {
				long user_me = rs.getLong("user_i");
				pstmtPutReco.setLong(1, user_me);
	...
	public String printLogs() {
		...
		try {
			PreparedStatement pstmtGetLogs = con
					.prepareStatement("SELECT * FROM `LogInLogs` WHERE `log`='snu' LIMIT 100;");
	...
	public void sendEmailAll() {
		try {
			PreparedStatement pstmtGetAllUsers = con.prepareStatement("SELECT * FROM `Users`;",
					ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			ResultSet rs = pstmtGetAllUsers.executeQuery();
			while (rs.next()) {
				String title = "Hi, " + rs.getString("id") + ". Now Recoeve.net is in beta test service stage.";
				String msg = "<span style='line-height:1.6; font-family:'Malgun Gothic', '맑은 고딕', 나눔고딕, NanumGothic, Tahoma, Sans-serif; font-size:20px'>안녕하세요? "
						+ rs.getString("id")
						+ " 님. 현재 Recoeve.net 은 거의 개발을 마쳐서 베터 서비스 단계에 있습니다.<br>다시 한번 방문 하셔서 Slow/Sexy/Sincere SNS 인 <a target=\"_blank\" href=\"https://recoeve.net/\">https://recoeve.net/</a> 서비스를 이용해보세요.<br><br>Hello, "
						+ rs.getString("id")
