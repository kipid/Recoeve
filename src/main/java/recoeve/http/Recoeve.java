...
public class Recoeve extends AbstractVerticle {
	...
	public void start() {
		...
		router2.route("/account/log-in/with/:authenticater").handler((RoutingContext ctx) -> {
			...
			if (pl.sessionPassed) {
				...
				switch (authenticater) {
					case "pre-google" -> {
						// e.g. path=/account/log-in/with/pre-google
					...
					case "google" -> {
						// e.g. path=/account/log-in/with/google
						pl.req.response()
					...
					case "google.do" -> {
						// e.g. path=/account/log-in/with/google.do
						pl.req.bodyHandler((Buffer data) -> {
							...
							if (db.getPreGoogle(inputs.get(1, "state"), pl.ip, pl.tNow)) {
								...
								List<io.vertx.core.http.Cookie> setCookieSSN = db.authUserWithGoogle(inputs, pl.ip, pl.userAgent, pl.tNow, pl.lang);
		...
		router1.get("/CDN/:fileName").handler((RoutingContext ctx) -> { // e.g. path=/CDN/icon-Recoeve.png
			...
			if (pl.refererAllowed) { // referer check.
				...
				if (fileName != null && !fileName.isEmpty()) {
					...
					pl.req.response().putHeader("ETag", "1.8.11");
					String[] fileNameSplit = fileName.split("\\.");
					switch (fileNameSplit[fileNameSplit.length - 1]) {
						case "jpeg", "jpg" -> { pl.req.response().putHeader("Content-Type", "image/jpeg"); }
						case "webm" -> { pl.req.response().putHeader("Content-Type", "video/webm"); }
						case "html" -> { pl.req.response().putHeader("Content-Type", "text/html; charset=utf-8"); }
						case "ico" -> { pl.req.response().putHeader("Content-Type", "image/x-icon"); }
						case "png" -> { pl.req.response().putHeader("Content-Type", "image/png"); }
						case "css" -> { pl.req.response().putHeader("Content-Type", "text/css; charset=utf-8"); }
						case "map" -> { pl.req.response().putHeader("Content-Type", "application/json; charset=utf-8"); }
						case "js" -> { pl.req.response().putHeader("Content-Type", "text/javascript; charset=utf-8"); }
						default -> { pl.req.response().putHeader("Content-Type", "text/plain; charset=utf-8"); }
					}
		...
		router.post("/get-redirect-hashpath").handler((RoutingContext ctx) -> {
			...
			pl.req.bodyHandler((Buffer data) -> {
				pl.req.response()
					.end("https://" + HOST + "/redirect/" + db.getRedirectHashpath(data.toString()), ENCODING);
				...
				if (fileName != null && !fileName.isEmpty()) {
					switch (fileName) {
						...
						default -> {
							...
							String fileInMemory = fileMap.getFileWithLang(fileName, pl.lang);
							if (fileInMemory != null) {
								pl.req.response().end(fileInMemory, ENCODING);
								msg = "Sended file in memory: " + fileName;
								System.out.println(msg);
								// pLHtml.append(msg + "</div>");
								// db.putLogAccess(pl.tNow, pl.user_i, pLHtml.toString(), db.getLogAccess(pl.tNow));
			...
			if (pl.refererAllowed) { // referer check.
				...
				if (finalUserId != null && !finalUserId.isEmpty() && db.idExists(finalUserId)) {
					...
					if (wildcard != null) {
						switch (wildcard) {
							case "get-Recos" -> {
								// e.g. path=/user/kipid/get-Recos
								pl.req.bodyHandler((Buffer data) -> {
									pl.req.response()
										.end(db.getRecos(finalUserId, new StrArray(data.toString())).replaceAll("%20", "%2520"), ENCODING);
							...
							case "get-UriList" -> {
								// e.g. path=/user/kipid/get-UriList
								pl.req.bodyHandler((Buffer data) -> {
									pl.req.response()
							...
							case "change-order-of-UriList" -> {
								// e.g. path=/user/kipid/change-order-of-UriList?cat=......
								if (pl.sessionPassed) {
							...
							case "get-Neighbors" -> {
								// e.g. path=/user/kipid/get-Neighbors
								pl.req.bodyHandler((Buffer data) -> {
									pl.req.response()
										.end(db.getStrOfNeighbors(finalUserId, data.toString(), pl.tNow), ENCODING);
							...
							case "cut-Neighbors" -> {
								// e.g. path=/user/kipid/cut-Neighbors
								if (pl.sessionPassed) {
									pl.req.bodyHandler((Buffer data) -> {
										pl.req.response()
											.end(db.cutNeighbors(finalUserId, Long.parseLong(pl.cookie.get("I"), 16), data.toString(), pl.tNow), ENCODING);
										String msg1 = "Sended neighbors.";
										System.out.println(msg1);
										// pLHtml.append(msg1 + "</div>");
									});
							...
							case "get-Recoms" -> {
								// e.g. path=/user/kipid/get-Recoms
								pl.req.bodyHandler((Buffer data) -> {
									pl.req.response()
		...
		router.routeWithRegex("^\\/account\\/([^\\/]+)(?:\\/([^\\/]+)\\/([^\\/]+))?$").handler((RoutingContext ctx) -> { // e.g.
			...
			if (pl.refererAllowed) { // referer check.
				...
				switch (param0) {
					...
					case "log-out", "log-out-from-all" -> {
						// path=/account/log-out or /account/log-out-from-all
						pl.req.response().putHeader("Content-Type", "text/html; charset=utf-8");
					...
					case "log-out.do" -> {
						...
						pl.req.response()
							.end(FileMap.replaceStr("[--Log-out : All log-in | session in server are deleted.--]", pl.lang), ENCODING);
						msg = "Sended log-out msg.";
						System.out.println(msg);
						// pLHtml.append(msg + "</div>");
						// db.putLogAccess(pl.tNow, pl.user_i, pLHtml.toString(), db.getLogAccess(pl.tNow));
					}
					case "log-out-from-all.do" -> {
						// path=/account/log-out-from-all.do
						pl.req.response().putHeader("Content-Type", "text/plain; charset=utf-8");
						...
						pl.req.response()
							.end(FileMap.replaceStr("[--Log-out : All log-in | session in server from all devices are deleted.--]", pl.lang), ENCODING);
							...
							pl.req.bodyHandler((Buffer data) -> {
								...
								if (db.checkAuthToken(inputs, pl.ip, pl.tNow)) {
									...
									if (db.createUser(inputs, pl.ip, pl.tNow, pl.lang)) {
										...
										pl.req.response()
											.end(FileMapWithVar.getFileWithLangAndVars("signed-up.html", pl.lang, varMap), ENCODING);
		...
		router.post("/account/log-in/remember-me.do").handler((RoutingContext ctx) -> { // path=/account/log-in/remember-me.do
			...
			pl.req.bodyHandler((Buffer data) -> {
				...
				if (!setCookieRMB.isEmpty() && setCookieRMB.get(0).getName().equals("I")) {
					...
					pl.req.response()
						.end(FileMap.replaceStr("Rmbd: [--Sended Rmbd with Set-Cookie of session and new rmbd token. (Succeed in remembering the user.)--]", pl.lang), ENCODING);
					...
					pl.req.response()
						.end(FileMap.replaceStr("[--Remembering you failed.--] [--Refresh the page and try again.--]", pl.lang), ENCODING);
