...
router.get("/CDN/:fileName").handler(ctx -> { // e.g. path=/CDN/icon-Recoeve.png
	...
	if (PrintLog.refererAllowed) { // referer check.
		...
		if (fileName!=null&&!fileName.isEmpty()) {
			...
			req.response().sendFile(fileName); // TODO: Cache file datas in memory to response faster.
			System.out.println("Sended "+fileName+".");
		}
		else {
			req.response().putHeader("Content-Type","text/plain; charset=utf-8");
			req.response().setStatusCode(404).end(INVALID_ACCESS, ENCODING);
...
router.get("/").handler(ctx -> { // path=/
	...
	if (PrintLog.cookie.get("I")!=null||PrintLog.cookie.get("rmbdI")!=null) {
		req.response().putHeader("Content-Type", "text/html; charset=utf-8");
		req.response().end(FileMapWithVar.get("user-page.html", PrintLog.lang, PrintLog.db.varMapMyPage(PrintLog.cookie)), ENCODING); // to "/user/:userId". (Cookie owner's page)
		System.out.println("Sended user-page.html");
	}
	else {
		req.response().putHeader("Content-Type", "text/html; charset=utf-8");
		req.response().end(FileMap.get("log-in.html", PrintLog.lang), ENCODING); // to "/account/log-in".
		...
		if (fileName!=null&&!fileName.isEmpty()) {
			switch (fileName) {
				case "jquery.js": // e.g. path=/jquery.js
					...
					req.response().end(FileMap.get("jquery.js", PrintLog.lang), ENCODING);
					System.out.println("Sended jquery.js.");
					break;
				case "prepare.js": // e.g. path=/prepare.js
					req.response().putHeader("Content-Type","text/javascript");
					req.response().end(FileMap.get("prepare.js", PrintLog.lang), ENCODING);
					System.out.println("Sended prepare.js.");
					break;
				case "sessionIter": // e.g. path=/sessionIter
					String iter=db.sessionIter(PrintLog.cookie);
				...
				case "reco": // e.g. path=/reco
					req.response().putHeader("Content-Type", "text/html; charset=utf-8");
					req.response().end(FileMapWithVar.get("user-page.html", PrintLog.lang, db.varMapMyPage(PrintLog.cookie)), ENCODING);
		...
		switch (pathSplit[1]) {
			case "user": // e.g. path=/user/kipid/mode/multireco?cat=...
				...
				if (pathSplit.length==3||pathSplit.length==5) { // e.g. path=/user/kipid[/mode/multireco]?cat=...
					if (db.idExists(user)) {
						req.response().putHeader("Content-Type","text/html; charset=utf-8");
						req.response().end(FileMapWithVar.get("user-page.html", PrintLog.lang, db.varMapUserPage(PrintLog.cookie, user)), ENCODING);
						System.out.println("Sended user-page.html");
					}
					else {
						String res=FileMap.replaceStr("<h1>[--User does not exist.--]</h1>", PrintLog.lang);
						req.response().putHeader("Content-Type","text/html; charset=utf-8");
					...
					switch (pathSplit[2]) {
						...
						case "do": // path=/reco/do
							if (sessionPassed) {
								req.bodyHandler((Buffer data) -> {
									...
									String res=db.recoDo(Long.parseLong(PrintLog.cookie.get("I"), 16), recoStr);
									req.response().putHeader("Content-Type","text/plain; charset=utf-8");
									req.response().end(res);
						...
						case "put": // path=/reco/put
							if (sessionPassed) {
								req.bodyHandler((Buffer data) -> {
									final String recoStr=data.toString();
									String res=db.putReco(Long.parseLong(PrintLog.cookie.get("I"), 16), recoStr);
									req.response().putHeader("Content-Type","text/plain; charset=utf-8");
									req.response().end(res);
				...
				switch (pathSplit[2]) {
					case "changePwd": // path=/account/changePwd
						if (db.checkChangePwdToken(req.params(), now)) {
							req.response().putHeader("Content-Type","text/html; charset=utf-8");
							req.response().end(FileMap.get("changePwd.html", PrintLog.lang), ENCODING);
					...
					case "log-in": // path=/account/log-in
						if (pathSplit.length==3) {
							if (sessionPassed) {
								...
								req.response().putHeader("Content-Type","text/html; charset=utf-8");
								req.response().end(FileMap.get("remember-me.html", PrintLog.lang), ENCODING);
								System.out.println("Sended remember-me.html.");
							}
							else {
								req.response().putHeader("Content-Type","text/html; charset=utf-8");
								req.response().end(FileMap.get("log-in.html", PrintLog.lang), ENCODING);
							...
							case "remember-me.do": // path=/account/log-in/remember-me.do
								if (method==HttpMethod.POST) {
									req.bodyHandler((Buffer data) -> {
										...
										List<io.vertx.core.http.Cookie> setCookieRMB=db.authUserFromRmbd(PrintLog.cookie, inputs, ip);
					...
					case "log-out": // path=/account/log-out
						List<io.vertx.core.http.Cookie> setDelCookie=db.logout(PrintLog.cookie);
					...
					case "forgotPwd": // path=/account/forgotPwd
						if (method==HttpMethod.POST) {
							req.bodyHandler((Buffer data) -> {
								StrArray inputs=new StrArray(data.toString());
								String forgotPwd=db.forgotPwd(inputs, PrintLog.lang);
								req.response().putHeader("Content-Type","text/plain; charset=utf-8");
							...
							req.bodyHandler((Buffer data) -> {
								...
								if (db.checkAuthToken(inputs, ip, now)) {
									...
									if (db.createUser(inputs, ip, now)) {
										...
										req.response().putHeader("Content-Type","text/html; charset=utf-8");
										req.response().end(FileMapWithVar.get("signed-up.html", PrintLog.lang, varMap), ENCODING);
					...
					case "verify": // path=/account/verify/.....token
						if (sessionPassed) {
							...
							if (db.verifyUser(PrintLog.cookie.get("I"), pathSplit3, ip)) {
								// User is verified.
								req.response().putHeader("Content-Type","text/plain; charset=utf-8");
			...
			case "changeOrders": // e.g. path=/changeOrders/CatList
				...
				switch (pathSplit[2]) {
					case "CatList":
						req.bodyHandler((Buffer data) -> {
							final boolean res=db.changeOrdersCatList(Long.parseLong(PrintLog.cookie.get("I"), 16), data.toString());
							req.response().putHeader("Content-Type","text/plain; charset=utf-8");
							req.response().end(""+res);
